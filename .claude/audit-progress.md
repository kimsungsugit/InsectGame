...
**audit 라운드는 2026-07-17자로 종료 선언했다. 큐는 비어 있다.**

수렴 근거 (실측):
- `Assets/Scripts` 159개 중 **Covered 100개**
- 남아 있던 큐 11건은 **전부 score 0** — OnGUI/Update 표면이 없는 데이터·정의 파일이다
  (CaptureTriggerOptionsUI, ItemInventoryGridItem, ItemRarityTuningUIController 등)
- `audit_candidates.py`가 새로 찾아내는 후보는 **2개**(UI/UITheme.cs, UI/UITween.cs)뿐이고
  그것들도 score 0이다
- 지금까지 나온 P0는 전부 모달·계정·세이브 계열에서 나왔고 그 영역은 전부 훑었다

`- [ ]`가 0건이므로 `audit_flow_inject`·`audit_reminder` 두 훅이 침묵하고 자동 플로우가
멈춘다. **이번엔 그게 의도된 상태다** — 예전엔 큐를 아무도 안 채워서 멈춘 것이었고
(2026-05-27~07-17), 지금은 훑을 게 없어서 멈춘 것이다. 그 둘을 혼동하지 말 것.

### 다시 켜는 법

새 기능이 대량 유입되면 큐를 다시 채운다:

```
python -X utf8 .claude/scripts/audit_candidates.py --emit-md
```

이 스크립트는 Covered/아카이브에 이름이 없는 `.cs`를 프레임 할당·미캐싱 조회·구독 해제
누락·싱글턴 참조로 채점해 후보를 낸다. 하드코딩 목록이 없어 stale해지지 않는다.
출력을 이 섹션에 붙이면 자동 플로우가 되살아난다.
