...
| 항목 | 임계값 | 의미 |
|---|---|---|
| 천장 부재: N연차 Legendary 0개 | < 50% | 100연차 후에도 절반 넘게 0개면 운게임 |
| CashShopUI 표시 가격 vs Manager 정본 | 0건 불일치 | 표시값이 실제 차감과 다르면 결제 분쟁 |
| UI 확률 표기 = 코드 파생 | 파생 유지 + 리터럴 0건 | 하드코딩이 부활하면 공시 위반 |
| 실버 가성비 (브론즈 대비) | 가격 증가율 × 0.6 | 가격이 오른 만큼 기댓값이 안 오르면 함정 |
| 골드 가성비 (실버 대비) | 가격 증가율 × 0.6 | 동일 |
...
| **UI 가격 불일치 FAIL** | `CashShopUI`의 박스 카드 가격 리터럴을 `CashShopManager.gemPrice`에 맞춤. 장기적으로 확률처럼 코드 파생으로 전환 |
| **UI 확률 하드코딩 회귀 FAIL** | 확률 표기를 다시 `GachaBoxManager.GetRateText(boxId)` 파생으로 되돌린다 |
| **가성비 WARN** | 차상위 박스의 EV 증가가 가격 증가에 못 미친다. 확률 임계값 또는 가격 재조정 |
| **박스 가성비 역전 WARN** | 명백한 디자인 결함 — 차상위 박스의 가격당 EV가 차하위 미만. **조정 전에 출력의 실제 확률을 먼저 볼 것** |
| **골드 캔디보너스 WARN** | 보너스 캔디 범위 상향 또는 별도 보상 (가챠 티켓 등) |
| **가챠 샤이니 WARN** | `GachaResult`에 샤이니 필드 추가, `InsectEntity`와 같은 확률 적용 |

> 권장안에 구체적 수치를 적어두지 않는다. 한때 여기 "골드 Legendary 5%→7%"라고
> 적혀 있었다 — 실제 골드 Legendary는 **45%**였다. 사본을 근거로 낸 권고는 정반대였다.
> 조정폭은 그때그때 시뮬 출력의 실측값을 보고 정할 것.
