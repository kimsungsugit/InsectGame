...
| # | 항목 | 검출 대상 |
|---|------|----------|
| 1 | 리전 참조 정합성 | RegionManager/RegionTerrainBuilder/RegionMapUI/AudioManager의 case "..." switch가 CreateExpandedRegions 정의에 없는 ID 참조 시 FAIL |
| 2 | 리전 고아 | CreateExpandedRegions에 정의했지만 4개 참조 위치 모두에서 미참조 시 WARN |
| 3 | 리전 중복 정의 | CreateRegions(legacy) ↔ CreateExpandedRegions(active) 양쪽에 동일 regionId 정의 시 WARN |
| 4 | dead method 일반 검출 | PlaySceneBootstrap의 private 메서드 중 참조 0건 시 FAIL (Unity 콜백 제외) |
| 5 | CashShop rewardItemId ↔ 아이템 풀 | CashShopManager.shopItems의 rewardItemId가 CreateCaptureItems / 알려진 풀에 없으면 WARN |
| 6 | itemId 중복 | CreateCaptureItems ∩ CashShop shopItems 교집합 시 FAIL |
| 7 | Gacha 풀 ID ↔ displayName | gachaExclusives 풀에 있고 exclusiveDisplayNames에 없는 ID는 FAIL |
| 8 | Gacha displayName 고아 | exclusiveDisplayNames에 있고 풀에 없는 ID는 WARN |
| 9-11 | Gacha Bronze/Silver/Gold 확률 분포 | 누적 임계값 단조증가 + Legendary > 0 검증. 위반 시 FAIL |
| 12 | CashShop UI 박스 가격 ↔ Manager gemPrice | UI `DrawBoxCard` 가격 인자가 `CashShopManager.shopItems[].gemPrice`와 다르면 FAIL |
| 13 | CashShop UI 박스 확률 텍스트 ↔ Gacha 임계값 | UI rateText의 등급별 % vs `GachaBoxManager.GetXxxRarity` 누적 임계값 차이 > 0.01 시 FAIL |
...
| 위험 신호 | 권장 조치 |
|---|---|
| 리전 참조 정합성 FAIL | RegionManager/Terrain/MapUI/Audio switch에 등장하는 미정의 regionId를 CreateExpandedRegions에 추가하거나 switch에서 제거 |
| 리전 고아 WARN | 정의했는데 미참조 — 게임에 등장 안 하는 리전. 4개 참조 위치(switch/case) 추가 또는 정의 제거 |
| 리전 중복 정의 WARN | 보통 dead 메서드(CreateRegions)에 잔존. dead method 제거로 자동 해소 |
...
| Gacha 확률 분포 FAIL | 임계값 단조증가 위반 또는 Legendary 0% — `GetBronzeRarity`/`GetSilverRarity`/`GetGoldRarity` 정정 |
...
| 영역 | 주담당 | 부수 |
|---|---|---|
| `PlaySceneBootstrap.CreateExpandedRegions` 리전 정의 | game-designer | data-architect |
